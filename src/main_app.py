...
class AppGenerator:
    ...
    def generate_application(self, user_prompt, output_path, include_tests=False, create_docker=False, add_ci_cd=False):
        self._requirements_spec = self._build_requirements_spec(user_prompt, include_tests, create_docker, add_ci_cd)
        ...
        is_unified_framework = self._requirements_spec.get("unified_frontend_backend", False)
        ...
        if isinstance(self._architecture.get('dependencies'), list):
            for dep in self._architecture.get('dependencies'):
                dep_name = dep.get('name', '').lower() if isinstance(dep, dict) else str(dep).lower()
        ...
        if is_unified_framework:
            print(f"Using unified frontend/backend architecture for {framework or language}")
        ...
        if language == 'python' and not framework and has_flask:
            print("Detected Python backend with HTML/JS frontend - assuming Flask framework")
            framework = 'flask'
            if isinstance(tech_stack, dict):
        ...
        if language == 'python' and ('flask' in framework or 'django' in framework):
            print(f"Detected {framework.capitalize()} framework - integrating frontend into templates")
            self._adjust_architecture_for_templates('flask' if 'flask' in framework else 'django')
        elif language == 'php':
    ...
    def _adjust_architecture_for_templates(self, framework):
        ...
        if framework == 'flask':
            ...
            self._adjust_for_express(frontend_files, backend_files, other_files)
        else:
            self._adjust_for_generic(frontend_files, backend_files, other_files)
    ...
    def _adjust_for_django(self, frontend_files, backend_files, other_files):
        app_name = self._requirements_spec.get('app_name', 'main').lower().replace(' ', '_')
